...
fn parseWithSchema(self: *NewParser, comptime S: openapi.SchemaComponent) !S.api_type {
    var ret: S.api_type = .{};
    try self._parseSchema(&ret, S);
    return ret;
}

pub fn _parseSchema(self: *NewParser, r: anytype, comptime S: openapi.SchemaComponent) anyerror!void {
    switch (S.serde.sx) {
        .object => |o| switch (o) {
...
pub fn _parseProperty(self: *NewParser, r: anytype, comptime P: openapi.Property) anyerror!void {
    const fname = P.api_name orelse P.name;
    const field = &@field(r, fname);
    const T = @TypeOf(field.*);

    if (P.ref) |ref| {
        return self._parseSchema(field, ref);
    }
...
pub fn dict(self: *NewParser) ![:0]const u8 {
...
pub fn arrayStrings(self: *NewParser) ![:0]const u8 {
...
pub fn assocsShort(self: *NewParser) !*slurm.db.List(*slurm.db.Association) {
    if (.array_begin != try self.source.next()) return error.UnexpectedToken;
...
pub fn arrayContainerToList(self: *NewParser, comptime S: openapi.SchemaComponent) !*S.api_type {
    if (.array_begin != try self.source.next()) return error.UnexpectedToken;
...
pub fn innerParse(self: *NewParser, comptime T: type) !T {
...
fn nextAlloc(self: *NewParser, when: std.json.AllocWhen) !Token {
    return try self.source.nextAllocMax(self.allocator, when, self.source.input.len);
}

fn nextFieldName(self: *NewParser) !?[]const u8 {
    const token = try self.nextAlloc(.alloc_if_needed);
    return fieldNameFromToken(token);
}

pub fn container(self: *NewParser, r: anytype, comptime S: openapi.SchemaComponent) anyerror!void {
    ...
    while (true) {
        ...
        inline for (S.properties, 0..) |prop, i| {
            if (std.mem.eql(u8, prop.name, field_name)) {
                ...
                try self._parseProperty(r, prop);
                fields_seen[i] = true;
                break;
            }
        } else {
